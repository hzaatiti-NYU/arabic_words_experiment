...
var test = {
    type: jsPsychHtmlKeyboardResponse,
    stimulus: function () {
        var Preview = jsPsych.timelineVariable('Preview');
        var Position = jsPsych.timelineVariable('Position');
        var letterSpacing = jsPsych.timelineVariable('LetterSpacing');
        var scaleAmount = 1; // Adjust this value to control horizontal stretching/compression
        var fontSize = 50; // Adjusted font size
        var scaleYAmount = 1; // Keep vertical scaling at 1 to avoid distortion
       // var letterSpacing = "0"; // Dynamically adjusted letter spacing
        var wordHtml = `<div class="arabic-text" style="position: absolute; text-align: center; font-size: ${fontSize}px; letter-spacing: ${letterSpacing}em; transform: scaleX(${scaleAmount}) scaleY(${scaleYAmount});">${Preview}</div>`;
        var fixationCrossHtml = '<div style="position: absolute; bottom: 500px; text-align: center; left: 50%; transform: translateX(-50%); font-size:60px;">+</div>';

        if (Position === 'Left') {
          wordHtml = `<div class="arabic-text" style="position: absolute; bottom: 500px; left: 40%; transform: scaleX(${scaleAmount}) scaleY(${scaleYAmount}); font-size: ${fontSize}px; letter-spacing: ${letterSpacing}em;">${Preview}</div>`;
        } else if (Position === 'Right') {
          wordHtml = `<div class="arabic-text" style="position: absolute; bottom: 500px; right: 40%; transform: scaleX(${scaleAmount}) scaleY(${scaleYAmount}); font-size: ${fontSize}px; letter-spacing: ${letterSpacing}em;">${Preview}</div>`;
        } else if (Position === 'Bottom') {
          wordHtml = `<div class="arabic-text" style="position: absolute; bottom: 550px; left: 50%; transform: translateX(-50%) scaleX(${scaleAmount}) scaleY(${scaleYAmount}); font-size: ${fontSize}px; letter-spacing: ${letterSpacing}em;">${Preview}</div>`;
        } else if (Position === 'Top') {
          wordHtml = `<div class="arabic-text" style="position: absolute; bottom: 450px; left: 50%; transform: translateX(-50%) scaleX(${scaleAmount}) scaleY(${scaleYAmount}); font-size: ${fontSize}px; letter-spacing: ${letterSpacing}em;">${Preview}</div>`;
        }
        return `${wordHtml}${fixationCrossHtml}`;
    },
    timeline_variables: practiceStimuli,
    trial_duration: 500 // Corrected to 500 milliseconds (5 seconds)
};
